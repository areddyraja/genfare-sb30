...
+ (CustomBadge*) customBadgeIOS7WithString:(NSString *)badgeString withScale:(CGFloat)scale {
    return [[self alloc] initWithString:badgeString withStringColor:[UIColor whiteColor] withInsetColor:[UIColor redColor] withBadgeFrame:NO withBadgeFrameColor:nil withScale:scale withShining:NO];
}
...
-(void) drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect
{
	...
	//CGContextSetShadowWithColor(context, CGSizeMake(1.0,1.0), 3, [[UIColor blackColor] CGColor]); // Commented out for iOS 7 style
