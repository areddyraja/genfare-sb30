...
class GFRouteOptionTableViewCell: UITableViewCell {
    ...
    @IBOutlet weak var scrollView: GFRouteItemsScrollView!
    ...
    func attachRouteItems(route:GFRoute) {
        ...
        for i in 0...(route.legsList!.count - 1) {
            ...
            if leg.mode == Constants.TransitMode.Bus {
                attachBus(num: leg.routeNumber?.stringUpTo(count: Constants.Values.MAX_CHAR_NUMBER_FOR_BUS) ?? "")
            }
    ...
    func attachBus(num:String) {
        ...
        currentX = busImage.frame.origin.x + label.frame.size.width + itemDistance
    }
