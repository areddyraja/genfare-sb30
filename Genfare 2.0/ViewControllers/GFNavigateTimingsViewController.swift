...
class GFNavigateTimingsViewController: GFBaseViewController {
    ...
    override func viewDidLoad() {
        ...
        TripDataManager.getRoutsForLocations { (success) in
            ...
            if success {
                DispatchQueue.main.async {
                    self.filterResults(self.fastestBtn)
    ...
    @IBAction func filterResults(_ sender: GFMenuButton) {
        ...
        switch sender {
            case fastestBtn:
                print("fastest")
                TripDataManager.routesList = TripDataManager.routesList?.sorted(by: {$0.duration < $1.duration })
            case cheapestBtn:
                print("cheapest")
                TripDataManager.routesList = TripDataManager.routesList?.sorted(by: {$0.fare < $1.fare })
            case transfersBtn:
                print("min transfers")
                TripDataManager.routesList = TripDataManager.routesList?.sorted(by: {$0.transfers < $1.transfers })
            case minWalkBtn:
                print("min walk")
                TripDataManager.routesList = TripDataManager.routesList?.sorted(by: {$0.walkDistance < $1.walkDistance })
            default:
                print("default")
        }
...
extension GFNavigateTimingsViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.routeNumLabel.text = "Bus \(route.startBusNumber().stringUpTo(count: Constants.Values.MAX_CHAR_NUMBER_FOR_BUS))"
        cell.routeNumLabel.sizeToFit()
        cell.travelTimeLabel.text = "Travel Time: \(Int(route.duration/60)) min"
        if (route.departsIn() < 10) {
            cell.depInTime.text = route.departsInStr()
        ...
        if (cell.depInTime.text?.count)! > 2 {
            cell.depUnits.isHidden = true
        }else{
            cell.depUnits.isHidden = false
        }
...
extension String {
    func stringUpTo (count:Int) -> String {
        var newTxt:String = self
        if newTxt.count > count {
            let upperBound = String.Index(encodedOffset: count)
            newTxt = String(newTxt[newTxt.startIndex..<upperBound])
        }
