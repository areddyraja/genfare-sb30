...
class GFNavigateViewController: GFBaseViewController, UITextFieldDelegate {
    ...
    @IBAction func homeButtonAction(_ sender: UIButton) {
        ...
        if let homeAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_HOME) {
            let components = homeAddress.components(separatedBy: "|")
            currentTextField?.text = components.first;
            updateLocationSelection(str: Constants.Address.Home)
        }else {
            popupAlert(title: "Alert", message: "No Saved Address Found.", actionTitles: ["OK"], actions: [nil])
        }
    }
    
    @IBAction func schoolButtonAction(_ sender: UIButton) {
        if let schoolAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_SCHOOL) {
            let components = schoolAddress.components(separatedBy: "|")
            currentTextField?.text = components.first;
    ...
    @IBAction func workButtonAction(_ sender: UIButton) {
        if let workAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_WORK) {
            let components = workAddress.components(separatedBy: "|")
            currentTextField?.text = components.first;
    ...
    func updateLocationSelection(str:String) {
        ...
        switch str {
            case Constants.Address.Work:
                let workAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_WORK)
                let comps = workAddress?.components(separatedBy: "|")
                location = GFLocation(coordinates: CLLocationCoordinate2D(latitude: Double(comps![1])!, longitude: Double(comps![2])!))
            case Constants.Address.School:
                let schoolAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_SCHOOL)
                let comps = schoolAddress?.components(separatedBy: "|")
                location = GFLocation(coordinates: CLLocationCoordinate2D(latitude: Double(comps![1])!, longitude: Double(comps![2])!))
            case Constants.Address.Home:
                let homeAddress = Utilities.getValueFromDefaults(forKey: KEY_SAVED_ADDRESS_HOME)
                let comps = homeAddress?.components(separatedBy: "|")
                location = GFLocation(coordinates: CLLocationCoordinate2D(latitude: Double(comps![1])!, longitude: Double(comps![2])!))
            default:
                print("default")
        }
...
extension GFNavigateViewController: UITableViewDelegate {
...
extension GFNavigateViewController: UITableViewDataSource {
...
extension GFNavigateViewController: MKLocalSearchCompleterDelegate {
