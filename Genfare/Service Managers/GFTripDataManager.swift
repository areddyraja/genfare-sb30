        ...
        if json["plan"]["itineraries"].count > 0 {
            ...
            for i in 0...(json["plan"]["itineraries"].count-1) {
                ...
                if json["plan"]["itineraries"][i]["legs"].count > 0 {
                    ...
                    route.startTime = json["plan"]["itineraries"][i]["startTime"].number
                    route.endTime = json["plan"]["itineraries"][i]["endTime"].number
                    route.duration = json["plan"]["itineraries"][i]["duration"].float
                    route.fare = json["plan"]["itineraries"][i]["fare"]["fare"]["regular"]["cents"].float
                    route.transfers = json["plan"]["itineraries"][i]["transfers"].int!
                    route.walkDistance = json["plan"]["itineraries"][i]["walkDistance"].float
